...
`;const st=()=>{const L=()=>{const v=window.innerWidth<=768;document.querySelectorAll(".hotspot").forEach(u=>{u.dataset.basePosition||(u.dataset.basePosition=u.getAttribute("data-position"));const x=u.dataset.basePosition.split(" ").map(Number);if(v){let B=x[0]*.25,y=x[1];u.slot==="hotspot-head"&&y>1.7&&(y=1.7),u.setAttribute("data-position",`${B} ${y} ${x[2]}`)}else u.setAttribute("data-position",u.dataset.basePosition)})};L(),window.addEventListener("resize",L),o.timeline({easing:"easeOutExpo"}).add({targets:".model-wrapper",scale:[.8,1],opacity:[0,1],duration:1200,delay:300});const k=document.querySelector(".proyectos");document.querySelectorAll(".card").length,window.anim1=o({targets:".anim-1 .shape",translateX:function(){return o.random(-80,80)},translateY:function(){return o.random(-80,80)},scale:function(){return o.random(.5,1.5)},rotate:function(){return o.random(-360,360)},duration:function(){return o.random(2e3,4e3)},easing:"easeInOutQuad",direction:"alternate",loop:!0,autoplay:!1}),window.anim2=o({targets:".anim-2 .sphere-paths circle, .anim-2 .sphere-paths ellipse",strokeDashoffset:[o.setDashoffset,0],easing:"easeInOutSine",duration:1e3,delay:function(v,c){return c*150},autoplay:!1});const n={grid:[13,13],from:"center"};window.anim3=o.timeline({loop:!0,autoplay:!1}).add({targets:".anim-3 .dot",scale:[{value:.1,easing:"easeOutSine",duration:500},{value:1,easing:"easeInOutQuad",duration:1200}],delay:o.stagger(100,n)}),window.anim4=o({targets:".anim-4 .radar-needle",rotate:[0,360],duration:1e3,easing:"linear",autoplay:!1});const r=document.querySelector(".contact-section"),h=document.querySelector(".folder-wrapper"),D=document.querySelector(".folder-front"),N=document.querySelectorAll(".contact-card");window.contactTl=o.timeline({autoplay:!1,easing:"linear"}),r&&h&&window.contactTl.add({targets:h,translateY:[100,0],opacity:[0,1],duration:500},0).add({targets:D,rotateX:["0deg","-20deg"],translateY:[0,10],scaleY:[1,.9],duration:500},300).add({targets:N,translateY:[0,-250],translateX:function(v,c){return[-30,-10,10,30][c]},rotate:function(v,c){return[-10,-5,5,10][c]},duration:800,delay:o.stagger(150)},600).add({targets:N,translateY:[-250,220],translateX:function(v,c){const u=window.innerWidth>768?120:70;return-(u*1.5)+u*c},rotate:0,scale:1,duration:800,delay:o.stagger(150)},1400),window.addEventListener("scroll",()=>{const v=window.pageYOffset,c=window.innerHeight,x=document.querySelector(".hero").offsetHeight,B=x>c?x-c:c;let y=v/B;y=Math.max(0,Math.min(1,y));let C=Math.max(0,1-y/.25);C=Math.min(1,C);const I=document.querySelector(".intro");I&&(I.style.opacity=C,I.style.transform=`translateY(${-50*(1-C)}px)`,I.style.pointerEvents=C===0?"none":"auto");let F=(y-.1)/.6;F=Math.max(0,Math.min(1,F));const E=1-Math.pow(1-F,3);let W=0;const V=document.querySelector(".hero-container"),$=document.querySelector(".model-wrapper"),G=document.querySelector("model-viewer");if(window.innerWidth>768){const s=V.offsetWidth,d=64,l=((s-d)/2+d)/2;if(W=-l*E,$){const p=l*2*E;$.style.width=`calc(100% + ${p}px)`,W=-(l+p/2)*E,$.style.transform=`translateX(${W}px)`}}const Q=1+.5*E;G&&(G.style.transform=`scale(${Q})`);const U=document.querySelectorAll(".hotspot"),_=.6/U.length;U.forEach((s,d)=>{const a=.35+d*_;y>=a?s.classList.contains("visible")||s.classList.add("visible"):s.classList.contains("visible")&&s.classList.remove("visible")});const J=k.offsetTop,K=k.offsetHeight;let H=(v-J)/(K-c);H=Math.max(0,Math.min(1,H));const z=H*100,O=document.querySelector(".progress-red");if(O){const s=Math.min(z,33.33);O.style.strokeDasharray=`${s} 100`,O.style.strokeDashoffset=0,O.style.opacity=s>0?1:0}const j=document.querySelector(".progress-green");if(j){const s=Math.max(0,Math.min(z-33.33,33.33));j.style.strokeDasharray=`${s} 100`,j.style.strokeDashoffset=-33.33,j.style.opacity=s>0?1:0}const Y=document.querySelector(".progress-blue");if(Y){const s=Math.max(0,Math.min(z-66.66,33.34));Y.style.strokeDasharray=`${s} 100`,Y.style.strokeDashoffset=-66.66,Y.style.opacity=s>0?1:0}(window.innerWidth<=768?[Array.from(document.querySelectorAll(".card"))]:Array.from(document.querySelectorAll(".column")).map(s=>Array.from(s.querySelectorAll(".card")))).forEach(s=>{const d=s.length;s.forEach((a,l)=>{const p=1/d,S=l*p;let w=1,t=1,i=0,e=0,g=d-l;const A=l===d-1,P=A?0:.2;let m=(H-S)/p;const b=a.querySelector(".card-image"),T=a.querySelector(".card-image-container"),R=Math.max(0,b.offsetHeight-T.offsetHeight);if(m<0){let f=-m;w=1-f*.05,i=f*60,t=1-f*.1,g=d-l,e=0}else if(m>=0&&m<=1-P){w=1,i=0,t=1,g=d-l+10;let f=1-P===0?1:m/(1-P);A&&(f=m),e=f*-R}else if(!A&&m>1-P&&m<=1){let f=(m-(1-P))/P;w=1-f*.05,i=-f*40,t=1-f,g=d-l+10,e=-R}else A||(w=.95,i=-40,t=0,g=0,e=-R);o({targets:a,scale:w,translateY:i,opacity:t,zIndex:g,duration:50,easing:"linear"}),o({targets:b,translateY:e,duration:50,easing:"linear"})})});const X=document.querySelector(".code-features");if(X){const s=X.offsetTop,d=X.offsetHeight;let a=(v-s)/(d-c);a=Math.max(0,Math.min(1,a));const l=document.querySelectorAll(".feature-item"),p=l.length;l.forEach((t,i)=>{const e=1/p,g=i*e,A=(i+1)*e,P=(a-g)/(e*.3),m=(a-(A-e*.3))/(e*.3);let b=0,T=100;a>=g&&a<A?(t.classList.add("active"),a<g+e*.3?(b=Math.min(1,Math.max(0,P)),T=100*(1-b)):a>A-e*.3?(b=Math.min(1,Math.max(0,1-m)),T=-100*m):(b=1,T=0)):(t.classList.remove("active"),b=0),o({targets:t,opacity:b,translateY:T,duration:50,easing:"linear"})});let S=0;if(l.forEach((t,i)=>{t.classList.contains("active")&&(S=i)}),document.querySelectorAll(".anim-container").forEach((t,i)=>{i===S?t.classList.add("active-anim"):t.classList.remove("active-anim")}),window.anim1&&(S===0?window.anim1.play():window.anim1.pause()),window.anim2&&S===1){const t=1/p,i=1*t;let e=(a-i)/t;e=Math.max(0,Math.min(1,e)),window.anim2.seek(window.anim2.duration*e)}if(window.anim3&&(S===2?window.anim3.play():window.anim3.pause()),window.anim4&&S===3){const t=1/p,i=3*t;let e=(a-i)/t;e=Math.max(0,Math.min(1,e)),window.anim4.seek(window.anim4.duration*e)}const w=document.querySelector(".code-mockup");if(w){o({targets:w,translateY:-50*a,duration:50,easing:"linear"});const t=w.querySelector(".language-js");if(t){t.dataset.fullHtml||(t.dataset.fullHtml=t.innerHTML,t.innerHTML="");const i=t.dataset.fullHtml;let e=(a-.1)/.8;e=Math.max(0,Math.min(1,e));const g=Math.floor(i.length*e);t.innerHTML=i.substring(0,g)}}}if(r&&window.contactTl){const s=r.offsetTop,d=r.offsetHeight,a=s-c,l=s+d-c;let p=(v-a)/(l-a);p=Math.max(0,Math.min(1,p)),window.contactTl.seek(window.contactTl.duration*p)}})},et=()=>{const L=document.querySelector(".typing-text"),M=document.querySelector("#preloader"),k=document.querySelector("#app"),q=document.querySelector(".cursor"),n=document.querySelector(".code-lines"),r=document.querySelector(".active-line"),h="php artisan serve",D={length:0};o({targets:D,length:h.length,round:1,easing:"linear",duration:1200,update:()=>{L.textContent=h.substring(0,D.length)},complete:()=>{setTimeout(()=>{q.style.display="none",r.style.display="none",n.innerHTML+=`
        ...
        `,setTimeout(()=>{o({targets:M,opacity:[1,0],duration:800,easing:"easeInOutQuad",complete:()=>{M.style.display="none",k.style.opacity=1,st()}})},2e3)},500)}})};et();
